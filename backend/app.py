...
def upload_file():
    """Handles file uploads, stores in GridFS, processes Markdown data, and adds to FAISS."""
    ...
    # Download the file from GridFS to a temp file for processing
    ...
    try:
        with tempfile.NamedTemporaryFile(suffix=ext, delete=False) as temp_file:
            temp_file.write(gridout.read())
            temp_file.flush()
            temp_file_path = temp_file.name

        # Convert the file to Markdown
        if ext == '.pdf':
            markdown_text = ai_core.convert_pdf_to_markdown(temp_file_path)
        elif ext == '.docx':
            markdown_text = ai_core.convert_docx_to_markdown(temp_file_path)
        elif ext == '.pptx':
            markdown_text = ai_core.convert_pptx_to_markdown(temp_file_path)
        elif ext == '.txt':
            markdown_text = ai_core.convert_txt_to_markdown(temp_file_path)
        else:
...
@app.route('/upload', methods=['POST'])
def upload_file():
    """Handles file uploads, processes Markdown data, embeds it, and adds it to FAISS."""
    logger.info("File upload request received.")
